...
- **🔴 NEXT — Tier 1 (extractor): clear the 6 residual quarantined reds.** These are GENUINE OCR extractor limits, not data (re-OCR is ineffective — same frame-segmentation defect class as 2582): (a) match-250 `pre_game_lobby_state_2` frames are scrambled (wrong gamertag↔position↔number → lobby gamertag 7/10, player_number 2/10, height wrong); (b) match-250 BGM/LW `is_captain` captain-icon misread (`consolidate` has no captain-confidence signal — needs evidence plumbing); (c) match-463 L2/L2.5/L3 below floor (dup-event / missing-field gaps). Fix at the extractor/segmentation + confidence layers; do NOT lower V2 / regression-floor expectations. See `docs/ocr/tier0-quarantined-worker-tests.txt` for the exact list + notes.
