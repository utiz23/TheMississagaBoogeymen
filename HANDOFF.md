...
- **🔴 NEXT — Tier 1, item 0 (data repair): un-quarantine the 7 pre-existing worker reds.** `docs/ocr/tier0-quarantined-worker-tests.txt` lists 7 tests red from LIVE match-250/463 data drift (e.g. 250 L2 actor-resolution regressed to 85.4% vs a 97.92% floor; 250 `bgm/LW captain` is a stale OR-vote leak). These pre-date Tier 0 (verified by stashing 0.1 + re-running). Repair the canonical data (re-OCR / re-consolidate / backfill-event-actor-resolution), re-anchor `L2_THRESHOLD` on the restored score, delete the matching quarantine lines, and confirm `verify-ocr.sh` goes fully green.
