/**
 * DORMANT — the account Server Actions, kept for the post-launch account
 * feature. Not wired to anything.
 *
 * The `'use server'` directive has been REMOVED, deliberately. It is what turns
 * a module's exports into Server Actions with stable, POST-able action ids, and
 * a dormant file must register none. Restoring it is part of re-enabling the
 * account system, not a tidy-up.
 *
 * The live module at `src/app/account-actions.ts` keeps these export names and
 * hard-refuses every one of them. See ./README.md.
 */
...
import { auth, requireAdmin } from './better-auth'
 ...
 * (apps/web/src/lib/account-system-disabled.test.ts) fails if anything under
 * apps/web/src — outside this dormant directory — references it by name.
 *
 * As of the pre-launch auth disable, the operator CLI named below is disabled
 * too and this whole module is dormant: see ./README.md and
 * apps/worker/src/deferred-auth/init-admin-cli.ts.
 */
