...
// Marker shape: every marker renders the SAME geometry regardless of side —
// a coloured outer shape, a white inner shape, and a black letter on top.
// Only the outer fill swaps per team. This keeps the rink readable when both
// clubs happen to share a similar primary colour (the white inner + the
// team-coloured rim still separates them visually) and gives every marker
// the same crisp legibility against the dark broadcast rink.

export function GoalMarker({ side, size = 18, className, awayColor, homeColor }: MarkerProps) {
  const ACCENT = pickColor(side, homeColor, awayColor)
  return (
    <svg
...
export function HitMarker({ side, size = 18, className, awayColor, homeColor }: MarkerProps) {
  ...
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <rect fill={ACCENT} width="34.45" height="34.45" />
