...
- **[WARN] Recommendation is partly overfit.** The "6 marker-collision pairs" are a specific match-463 symptom. Event-first seeding (Rec 1) does generalise — it's a structural improvement. But the *clock-aware clustering* part assumes events are well-separated in time; in NHL 26 a faceoff and a shot can occur within 1-2 seconds at the same hockey coordinate. The research notes "small tolerance" but doesn't define it. A 2-second clock tolerance would still collide many real match-event pairs; <1 second risks splitting genuine OCR jitter. Needs empirical tuning on a non-463 match.
