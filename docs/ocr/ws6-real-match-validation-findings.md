...
> _second_ match is still wanted for true generalization (fix trained/validated on 2582 only).
...
nhl26 anchors are _menu_ tokens ("world of chel", "loadouts", …) that never appear in gameplay. This is
...
| 1967–1971 | `lt goalsummary / shotsummary / faceoffsummary`    | unknown_or_transition   | **post*game_box_score*{goals,shots,faceoffs}** |
...
- **`color_score = 0.0` across the entire post-game** (dark-grey UI). The classifier appears to lean on
  ...
  regex/anchor priors that _should_ fire on `endofgame`/`goalsummary`/`allevents…period` do not override
  it.
...
- **Bucket:** MODEL WEAKNESS (classifier weights / anchor-prior wiring). **Severity:** HIGH — it blocks the
  ...
  color-feature dependence on dark screens. If the anchor priors are _supposed_ to force these classes and
