...
**Goal:** turn "operational, all gates red" into "operational and verified" — an enforced, green, _honest_ quality
...
1. **`validateCandidateRun` is NOT the quality gate.** It only checks promotion floors + fatal extractor errors
   ...
   quality gate is `overall_pass` from `computeLayers`, and _that_ is what must block activation — `validate` is a
   cheap structural pre-check, not the gate.
2. **`overall_pass` is a post-activation measurement by construction.** `computeLayers` reads the _canonical_ tables
...
Because `overall_pass` can only be computed _after_ the canonical rebuild, restructure `activate`
...
2. **Inside the existing activation transaction** (the one that flips `is_active` + `rebuildCanonicalsFromActiveRun`,
   ...
   gate roll back _every_ activation.
3. **Fail CLOSED — require an affirmative pass, not merely `!== false`.** `buildReportBody` is fail-soft: `safeCall`
   ...
   `overall.pass === false` check would let a _scoring failure_ activate. The gate must therefore throw (→ roll back
...
4. **`--force --reason "<text>"` escape hatch:** bypasses _both_ checks, commits, and **persists the override** (see
...
1. **Measure first:** `pnpm --filter worker run-quality --run-id N --json` for the 4 active runs; tabulate
   l2 / l2_lineup / l3.
2. **Pick the reference run(s)** that are _independently trustworthy_ — match 250 is verified against the V2
...
1. **Prerequisite — the v2 classifier weights.** The proving bench needs
   ...
   committed) _or_ document `tools/game_ocr/scripts/train_screen_classifier.py --engine viterbi_v2` as a verify
