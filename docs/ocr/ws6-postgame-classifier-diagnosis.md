...
post-game states and (b) the classifier has no post-game text *feature* — so it must discriminate
...
frames, and the Viterbi decoder smooths away the sparse frames the classifier *does* get right.
...
The state machine *does* define post-game `anchor_substrings`
...
3. **Assertions:** per-clip ≥90% per-frame accuracy **plus** a hard rule mirroring match-968's
   contamination gate — *no labeled post-game span may decode as `unknown_or_transition` or
   `player_loadout_view`*. This is exactly the failure WS6 hit.
