...
- [x] **Step 8 (ON-BOX) — DONE (2026-07-15, GPU box). ALL proof points met on real GPU + live DB, and 4.5 Step 3's two extra points with them. `batch-promote` is PROVEN end-to-end.** Target: `2026-06-18_17-29-58.mkv` (sha `fe0e7bf6ac15`) → **match 2675**.
  - **🚩 THE TARGET THIS STEP NAMED WAS UNUSABLE — do not retry it.** `2026-05-08_18-25-42.mkv` (→ match 250) is the **pilot**: its sha `a55b2ebf` is already in `ocr_capture_batches` ⇒ `_process_target` hits `already_ingested` and returns. Worse, **`--limit` slices the plan BEFORE that skip**, so `batch --limit 1` would have burned its only slot on a no-op and proved nothing. **`batch` has no per-video flag** — the only targeting lever is `--since` + priority order. Chosen via: `--since 2026-06-18` ⇒ 12 targets / 19.1 GB to hash, first = `2026-06-18_17-29-58.mkv` (single-match, priority 1, sha not among the 7 ingested; match 2675 had 0 extractions / 0 summaries / 0 batches / 0 runs).
  - **Step 0's sizing method re-derived and CONFIRMED re-runnable** (its script was throwaway): real `enumerate_targets` → ffprobe → EA `matches` in `[stamp-2m, stamp+dur+5m]`, `game_title_id=1`. Reproduced the recorded distribution **exactly** (1→35, 2→22, 3→9, 4→1, 5→2, 6→1, 0→8; 35/78 single-match).
  - **✅ PROOF POINTS (Step 8):** `pass1: 69 segments (0.0s)` + `pass2: 10 segment dirs, 382 frames (0.0s)` — **cache hit** (a fresh Pass-1 took **2682.2s**, so 0.0s is conclusive; no flag drift, no `CacheMismatch`) · **NO "dispatch deferred" line** (branch (c) taken) · `[dispatch] 10 ok, 0 failed in 536.6s` · **4 `match_period_summaries` rows** landed for 2675 (`source=ocr`, `review_status=pending_review`) + 239 `ocr_extractions` + 10 capture batches · `[promote] done: 1 promoted, 0 failed — PASS=1 HOLD=0 OPERATOR_CONFIRM=0 ERROR=0` · crash-safe summary persisted (`completed:true`).
  - **✅ CONVERGENCE:** the follow-up `batch-promote --dry-run` returns **`nothing to drain: 2 video(s) confirmed, every confirmed reel already dispatched` → 0 videos in 2.6s** (early-return before touching disk — no 19 GB re-hash). The drain does not loop.
  - **✅ FINDING 1 CORROBORATED ON-BOX:** match 2675 has **0 `ocr_decoder_runs`** after a full fresh-ingest+promote ⇒ `run-quality --emit-row` is indeed structurally unreachable for these matches. The `match-quality` route worked: `[promote] match 2675: PASS`.
  - **🚩 `PASS` CERTIFIES THE FINAL SCORE ONLY — it is NOT "this match's OCR data is clean".** 2675 graded **PASS** (`finalAccuracy=1`, TOT-row final 2-5 == API truth) while its **L4 `score=0.167` (1/6 fields)**: `periodCoverage=0.75` (P2 goals-against never read), per-period P3 GA read **7** in a 5-goal game, and team totals mismatch on goalsAgainst (8 vs 5 — the period sum), shotsFor (18 vs 16), shotsAgainst (22 vs 17), faceoffsFor (18 vs 7). `periodAccuracy` correctly `null` (coverage < 1 ⇒ the 973/974 confound removal working as designed). **This is per spec** (4.G grades the TOT final; Finding 2 says the verdict routes, never withholds) — but it means a PASS match is routed AWAY from review while carrying badly misread per-period rows. **Calibration question for the corpus run, not a bug.**
  - **🚩 THE BATCH PATH NEEDS `DATABASE_URL` AND FAILS ASYMMETRICALLY WITHOUT IT.** `_psql_query` shells to `docker exec psql` (works with no env), but every **worker CLI** subprocess needs `DATABASE_URL` ⇒ planning succeeds and execution dies. Load it: `set -a && source .env && set +a`. **The dangerous half:** `load_confirmed_reel_map` returns `{}` on **ANY** failure (best-effort by contract) and `orchestrator.py:825` maps `{} → None` ⇒ the **deferred** branch. In Pass 1 that is harmless (deferred is correct anyway — observed live). **In `batch-promote` the same swallow would silently skip dispatch and promote nothing** while the ingest exits 0. Here it was caught only because `_grade_match` also needs `DATABASE_URL` and raised. A reel-map failure that leaves grading working (stale `dist/`, pnpm off PATH, a reel-map bug) ⇒ **silent no-op drain.** Follow-up candidate: distinguish "no confirmations" from "lookup failed".
  - **🚩 THE VENV MUST BE ACTIVATED, NOT INVOKED BY PATH.** `_process_target` spawns bare `python3` ⇒ `./.venv/bin/python -m video_ingest.cli batch` makes `preflight()` PASS in-process and then the subprocess die on `ModuleNotFoundError: video_ingest` (`/usr/bin/python3`). Use `source tools/video_ingest/.venv/bin/activate` first.
  - **✅ UNPLANNED EVIDENCE — per-video isolation proven against TWO REAL failures** (not monkeypatched): the `ModuleNotFoundError` above and `propose` exiting 1 on the missing `DATABASE_URL` both logged `[batch] SKIP <name>: … failed (exit 1)` and the loop survived.
  - **Cosmetic:** the log reads **"1 reel need association"** — 4.5's pluralization fix corrected the noun but not the verb (this step expected "needs"). Harmless.
  - **Timings (RTX 3060):** fresh Pass-1 decode **2682.2s** (44.7 min, 2048 frames → 69 segments) · Pass-2 **540.1s** (10 segments, 382 frames) · re-run both **0.0s** (cache) · dispatch **536.6s** · promote wall ~9.4 min. The sha cache was **COLD** at session start (no `sha-cache.json`); it is now warm.
...
- [x] **Step 3 (ON-BOX) — DONE (2026-07-15, GPU box), executed inside 4.4 Step 8. 4.5 IS PROVEN ON REAL GPU: single-reel videos are no longer stranded.** Target `2026-06-18_17-29-58.mkv` → **match 2675** (NOT the match-250 video the plan named — it is the already-ingested pilot; see Step 8's 🚩).
  - **This was the FIRST time `batch` Pass-1 ever ran on a single-reel video** (Step 0: the stranding signature had never fired because batch had only run the 5-reel pilot) ⇒ the run is the genuine 4.5 regression scenario, not a rehearsal. `reels.json` came back `reel_count: 1` (`has_boxscore: true`, no completeness flags, `boundary_confidence: 1.0`).
