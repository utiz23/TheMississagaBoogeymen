...
- [ ] **Step 6 (ON-BOX, separate verify session)** — Manual verify (evidence, small + safe): `video-ingest batch --video-root … --dry-run --limit 3` prints the enumerated/deduped/prioritized plan without mutating. Then a single real `--limit 1` on one known multi-match file end-to-end; confirm reels.json + proposals land and nothing promotes without confirm. Also confirm the `resolve-match propose --identities <cache>/<sha>` path shape against a real dispatch. Record in `HANDOFF.md`. **The promotion-of-confirmed second pass (per-reel Pass-2 + `run-quality --emit-row` carrying 4.G's L4) is a bounded follow-up, not in this first-pass loop.**
