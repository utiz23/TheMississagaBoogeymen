...
- [ ] **Step 2** — Run; expect FAIL.
- [ ] **Step 3** — Implement the loop reusing `reprocess.py`'s `_run_streaming`/`_run_decoder_runs_cli`; register the `batch` Typer command in `cli.py`.
- [ ] **Step 4** — Run; expect PASS + full video_ingest suite green.
- [ ] **Step 5** — Commit: `git commit -am "feat(video-ingest): unattended batch run loop + review-queue gate"`.
- [ ] **Step 6** — Manual verify (evidence, small + safe): `video-ingest batch --dry-run --limit 3` prints the enumerated/deduped/prioritized plan without mutating. Then a single real `--limit 1` on one known multi-match file end-to-end; confirm reels.json + proposals + an L4 scorecard land, and nothing promotes without confirm. Record in `HANDOFF.md`.
