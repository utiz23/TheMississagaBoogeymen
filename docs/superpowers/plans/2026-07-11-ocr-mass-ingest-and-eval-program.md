...
### Task 4.5: Un-strand single-reel videos (**NEXT** — blocks a trustworthy full-corpus run)
...
- [ ] **Step 0: SIZE IT FIRST (cheap, read-only, do this before designing).** How many corpus videos are single-reel? The 7 already-ingested shas + any `reels.json` under `/tmp/ingest-cache/<sha>/` give a sample; `SELECT count(*) FROM ocr_extractions WHERE transform_status='error'` and the batch's own `--dry-run` plan bound it. **If the share is tiny, a narrower fix (or an operator `--match-id` override) may beat a design change.**
- [ ] **Step 1** — Decide the shape. Candidates: (a) emit a 1-reel identity file too (let ② associate single-match videos through the same path — most uniform, makes `batch-promote` the single drain); (b) have `run_batch` resolve + pass `--match-id` for the single-match case (narrower, but re-introduces the pre-association chicken-and-egg ② solved); (c) leave Pass-1 deferred for single reels as well. **(a) is the presumed favourite — confirm against `group_into_reels`' 1-reel semantics and branch (a)'s "single-match parity" contract before committing.**
- [ ] **Step 2+** — TDD per the established pattern; the ① reel tests in `tests/test_match_split.py` are the anchor. Watch for the branch-(a) parity contract (it deliberately forwards the orchestrator's `run_id`, unlike branch (c)'s forced `run_id=None`).
