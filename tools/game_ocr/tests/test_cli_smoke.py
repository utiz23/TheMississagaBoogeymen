...
class CLISmokeTests(unittest.TestCase):
    ...
    def test_extract_no_use_gpu_parity(self) -> None:
        # extract defaults to --use-gpu (④ Task 4.0); --no-use-gpu must keep
        # the CPU RapidOCR path working identically for parity.
        with tempfile.TemporaryDirectory() as tmpdir:
            output = Path(tmpdir) / "result.json"
            command = [
                "python3",
